-- [Mod]Fancy Pickets [fancy_pickets] [Ver. 0.1]
...
function fancy_pickets.register_fancy_picket_nodes(subname, groups, single_texture, description, sounds)
	...
	-- Straight Fancy Picket
	minetest.register_node(":fancy_pickets:fancy_picket" .. subname, {
		description = description .. " Fancy Picket",
		tiles = images,
		paramtype = "light",
		paramtype2 = "facedir",
		groups = node_groups,
		...
		node_box = {
			type = "fixed",
			fixed = {
				{0.4375, -0.5, -0.0625, 0.5, 0.375, 0},
				{-0.5, -0.5, -0.0625, -0.4375, 0.375, 0},
				{-0.3125, -0.5, -0.0625, -0.1875, 0.4375, 0},
				{-0.0625, -0.5, -0.0625, 0.0625, 0.5, 0},
				{0.1875, -0.5, -0.0625, 0.3125, 0.4375, 0},
	...
	-- Fancy Picket Post
	minetest.register_node(":fancy_pickets:fancy_post_picket" .. subname, {
		description = description .. " Fancy Picket Post",
		tiles = images,
		paramtype = "light",
		paramtype2 = "facedir",
		groups = node_groups,
	...
	-- Fancy Outer Corner
	minetest.register_node(":fancy_pickets:fancy_picket_corner" .. subname, {
		description = description .. " Fancy Picket Corner",
		tiles = images,
		paramtype = "light",
		paramtype2 = "facedir",
		groups = node_groups,
	...
	-- Fancy Inner Corner
	minetest.register_node(":fancy_pickets:fancy_picket_icorner" .. subname, {
		description = description .. " Fancy Picket iCorner",
		tiles = images,
		paramtype = "light",
		paramtype2 = "facedir",
		groups = node_groups,
		...
		node_box = {
			...
			fixed = {
				...
				{0, -0.5, -0.5, 0.0625, 0.375, -0.4375},
				{0, -0.5, -0.3125, 0.0625, 0.4375, -0.1875},
			}
		},
		selection_box = {
			type = "fixed",
...
function fancy_pickets.register_gate_nodes(subname, groups, gate_texture, post_texture, description, sounds)
	local node_groups = table.copy(groups)
	node_groups.picket = 1
	node_groups.gate = 1
	...
	-- Closed State
	minetest.register_node(":fancy_pickets:fancy_gate_closed" .. subname, {
		description = description .. " Fancy Gate",
		tiles = gate_images,
		paramtype = "light",
		paramtype2 = "facedir",
		...
		sounds = sounds,
		drawtype = "mesh",
		mesh = "fancy_picket_gate_closed.obj",
		selection_box = {
			type = "fixed",
		...
		on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			minetest.swap_node(pos, {name = "fancy_pickets:fancy_gate_half_open" .. subname, param2 = node.param2})
			minetest.sound_play("doors_fencegate_open", {pos = pos, gain = 1.0, max_hear_distance = 16})

			minetest.after(0.15, function()
				local current_node = minetest.get_node(pos)
				if current_node.name == "fancy_pickets:fancy_gate_half_open" .. subname then
					minetest.swap_node(pos, {name = "fancy_pickets:fancy_gate_open" .. subname, param2 = node.param2})
				end
			end)
		end,
		on_rotate = handle_manual_rotation,
	...
	minetest.register_node(":fancy_pickets:fancy_gate_half_open" .. subname, {
		description = description .. " Fancy Gate (Opening...)",
		tiles = gate_images,
		paramtype = "light",
		paramtype2 = "facedir",
		sunlight_propagates = true,
		...
		sounds = sounds,
		drop = "fancy_pickets:fancy_gate_closed" .. subname,
		drawtype = "mesh",
		mesh = "fancy_picket_gate_half_open.obj",
	...
	minetest.register_node(":fancy_pickets:fancy_gate_open" .. subname, {
		description = description .. " Fancy Gate (Open)",
		tiles = gate_images,
		paramtype = "light",
		paramtype2 = "facedir",
		sunlight_propagates = true,
		...
		drop = "fancy_pickets:fancy_gate_closed" .. subname,
		drawtype = "mesh",
		mesh = "fancy_picket_gate_open.obj",
		create_inventory_image = false,
		selection_box = {
			type = "fixed",
			fixed = {-0.5, -0.5, -0.125, 0.5, 0.5, 0.0625}
		...
		on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			...
			minetest.after(0.15, function()
				local current_node = minetest.get_node(pos)
				if current_node.name == "fancy_pickets:fancy_gate_half_open" .. subname then
					minetest.swap_node(pos, {name = "fancy_pickets:fancy_gate_closed" .. subname, param2 = node.param2})
				end
			end)
		end,
		on_rotate = handle_manual_rotation,
...
function fancy_pickets.register_all_variants(subname, groups, fence_tex, gate_tex, post_tex, desc, sounds)
	fancy_pickets.register_fancy_picket_nodes(subname, groups, fence_tex, desc, sounds)
	fancy_pickets.register_gate_nodes(subname, groups, gate_tex, post_tex, desc, sounds)
end
...
-- =================================
-- FANCY REGISTRATIONS WITH SETTINGS
-- =================================

local master_enable_fancy = minetest.settings:get_bool("enable_fancy_pickets", true)

if master_enable_fancy then
	if minetest.settings:get_bool("enable_picket_birch", true) then
		fancy_pickets.register_all_variants("_birch",
	...
	if minetest.settings:get_bool("enable_picket_maple", true) then
		fancy_pickets.register_all_variants("_maple",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
	...
	if minetest.settings:get_bool("enable_picket_oak", true) then
		fancy_pickets.register_all_variants("_oak",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
	...
	if minetest.settings:get_bool("enable_picket_pine", true) then
		fancy_pickets.register_all_variants("_pine",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
	...
	if minetest.settings:get_bool("enable_picket_walnut", true) then
		fancy_pickets.register_all_variants("_walnut",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
	...
	if minetest.settings:get_bool("enable_picket_white", true) then
		fancy_pickets.register_all_variants("_white",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
	...
	if minetest.settings:get_bool("enable_picket_willow", true) then
		fancy_pickets.register_all_variants("_willow",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
	...
	if minetest.settings:get_bool("enable_picket_wood", true) then
		fancy_pickets.register_all_variants("_wood",
			{snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, flammable = 3},
